...
<div id="<?=$itemIds['ID']?>">
	<div class="product">
		...
		<div class="product-right">
			...
			<span id="<?=$itemIds['BASKET_ACTIONS_ID']?>" style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none')?>;">
				<?
				if ($showAddBtn)
				{
					?>
					<a class="btn <?=$showButtonClassName?> add-bag go-to-bag" id="<?=$itemIds['ADD_BASKET_LINK']?>"
				...
				{
					?>
					<a class="btn <?=$buyButtonClassName?> add-bag go-to-bag" id="<?=$itemIds['BUY_LINK']?>"
						href="javascript:void(0);">
			...
			<? if (!empty($arResult["SLIDER_IMAGES"])) : ?>
				<div class="cusrousel-mini" data-entity="image">
