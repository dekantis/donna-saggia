...
if (isset($arResult['ITEM']))
{
	...
	?>
	<a href="<?
	if ($arParams['SEF_MOD'] == 'Y')
	{
		echo '/catalog/' . $item['IBLOCK_SECTION_ID'] . '/' . $item['ID'] . '/';
	}
	else
	{
		$item['DETAIL_PAGE_URL'];
	}?>" id="<?=$areaId?>">
		<span class="relative-image">
			<img src="<?=CFile::ResizeImageGet($morePhoto[0]['ID'], array('width'=>'80', 'height'=>'110'), BX_RESIZE_IMAGE_PROPORTIONAL , true)['src'];?>">
		</span>
			...
			else
			{
				...
				if (!empty($item["OFFERS"]))
				{
					...
					foreach($item["OFFERS"] as $offer)
					{
						...
						elseif(empty($minPrice) || $minPrice > $offer["ITEM_PRICES"]["PRICE"])
						{
							$minPrice = $offer["ITEM_PRICES"][0]["PRICE"];
							$minPricePrint = $offer["ITEM_PRICES"][0]["PRINT_PRICE"];
						}										
					}
				}
				echo $minPricePrint;
