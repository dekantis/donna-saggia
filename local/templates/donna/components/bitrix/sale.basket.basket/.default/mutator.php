...
foreach ($this->basketItems as $row)
{
	...
	if (!empty($row['PREVIEW_PICTURE']))
	{
		...
		$imageFile = CFile::GetFileArray($row['PREVIEW_PICTURE']);
		$rowData['IMAGE_URL'] = CFile::ResizeImageGet($imageFile, $size, BX_RESIZE_IMAGE_EXACT)["src"];
	}
...
foreach ($this->columns as $column) {
	...
	$name = Loc::getMessage("SBB_" . $column . "_NAME");
